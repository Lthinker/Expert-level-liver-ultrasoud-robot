...
def _fit(data: Union[torch.Tensor, np.ndarray, zarr.Array],
    ...
    if mode == 'limits': # zh: 用最大最小值来确定缩放因子和偏移量，以scale到output_max和output_min之间
        if fit_offset:
            # unit scale
